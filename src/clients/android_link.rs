...
//!
//! 声明是 Client 档案字段，不是注册/更新入参。只有实例 Owner 通过
//! `/api/v1/admin/app-links` 把 Client 发布到本 Issuer 主机；用户自助通道
//! 不存在。公开 `/.well-known/assetlinks.json` 发布这些已登记声明。
//! 不同包名各成一条 statement；同包名多 Client 的指纹合并到同一 statement。
//!
