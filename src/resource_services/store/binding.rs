...
impl super::Store {
    pub async fn lock_live_binding_for_user(
        ...
        crate::sqlx::query_as(
            "SELECT id, provider_id, user_id, uid, issuer, grant_id,
             ...
             FROM resource_service_bindings
    ...
    /// 兑换路径的非锁定读取：某用户在某资源服务上的 live 绑定。
    pub async fn find_live_binding(
        &self,
        provider_id: Uuid,
        user_id: UserId,
    ) -> Result<Option<BindingRow>, crate::sqlx::Error> {
...
async fn commit_bundle(
    ...
    crate::sqlx::query("SAVEPOINT resource_service_commit_bundle")
        .execute(&mut **tx)
        .await?;
    ...
    match updated {
        Ok(Some(row)) => {
            crate::sqlx::query("RELEASE SAVEPOINT resource_service_commit_bundle")
                .execute(&mut **tx)
                .await?;
            Ok(row)
        }
        ...
        Err(error) if unique_violation(&error, LIVE_PROVIDER_UID) => {
            crate::sqlx::query("ROLLBACK TO SAVEPOINT resource_service_commit_bundle")
                .execute(&mut **tx)
                .await?;
