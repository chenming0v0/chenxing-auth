...
use crate::resource_services::types::{OPERATION_LEASE, OperationRow, OutboxRow};
...
impl super::Store {
    pub async fn claim_operation(
        ...
        let created: Option<OperationRow> = crate::sqlx::query_as(
            "INSERT INTO resource_service_operations (
             ...
             ON CONFLICT ON CONSTRAINT resource_service_operations_pkey DO NOTHING
        ...
        let existing = crate::sqlx::query_as(
            "SELECT idempotency_key, provider_id, binding_id, user_id, operation_type,
                    status, lease_until, request_fingerprint, provider_revision,
                    created_at, committed_at
             FROM resource_service_operations
    ...
    pub async fn reclaim_expired_lease(
        ...
        crate::sqlx::query_as(
            "UPDATE resource_service_operations
             SET status = 'leased',
    ...
    pub async fn insert_revocation_outbox(
        ...
        let inserted: Option<OutboxRow> = crate::sqlx::query_as(
            "INSERT INTO resource_service_revocation_outbox (
                 id, provider_id, binding_id, user_id
             ) VALUES ($1, $2, $3, $4)
             ON CONFLICT ON CONSTRAINT resource_service_revocation_outbox_binding_key DO NOTHING
        ...
        crate::sqlx::query_as(
            "SELECT id, provider_id, binding_id, user_id, attempts,
                    available_at, processed_at, last_error, created_at
             FROM resource_service_revocation_outbox
             WHERE binding_id = $1
             FOR UPDATE",
        )
        .bind(binding_id)
    ...
    pub async fn claim_revocation_outbox(
        ...
        crate::sqlx::query_as(
            "WITH next AS (
             ...
             UPDATE resource_service_revocation_outbox AS outbox
    ...
    pub async fn mark_outbox_processed(
        ...
        let updated = crate::sqlx::query(
            "UPDATE resource_service_revocation_outbox
