...
//! 与旧 provider/SMTP 主密钥不同文件，互不覆盖。
//!
//! 用途隔离由 [`SecretContext`] 的类型化变体表达：
//!
//! - 提供方配置：`SecretContext::ResourceServiceProvider(provider)`（仅 provider UUID）
//! - 绑定令牌包：`SecretContext::ResourceServiceBindingToken { provider, binding }`
//! - 操作上下文：`SecretContext::ResourceServiceOperation { provider, binding }`
//! - 撤销任务：`SecretContext::ResourceServiceRevocation { provider, binding }`
//!
//! 旧变体（`Provider` / `Smtp`）的 AAD 字节完全不变。
//!
