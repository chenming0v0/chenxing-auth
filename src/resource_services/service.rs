...
pub use scope_input::{ScopeDeclaration, default_scope};

#[derive(Clone)]
pub struct ResourceServiceService {
...
impl ResourceServiceService {
    ...
    fn encrypt_provider_secret(
        ...
        Ok(encrypt_secret(
            &self.secrets,
            SecretContext::ResourceServiceProvider(provider_id),
            secret,
        )?
        .encode())
    }
    ...
    fn decrypt_provider_secret(&self, row: &ProviderRow) -> Result<SecretString, ServiceError> {
        ...
        Ok(decrypt_secret(
            &self.secrets,
            SecretContext::ResourceServiceProvider(row.id),
            &ciphertext,
        )?)
    }
