...
impl ResourceServiceService {
    ...
    async fn commit_created(
        &self,
        credential: UserSessionCredential,
        provider: &crate::resource_services::types::ProviderRow,
        binding: &crate::resource_services::types::BindingRow,
        operation: &crate::resource_services::types::OperationRow,
        bundle: TokenBundle,
    ) -> Result<BindingView, ServiceError> {
        let now = self.clock.now();
        let mut tx = self.pool.begin().await?;
