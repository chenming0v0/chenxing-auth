...
fn purpose_and_identity_are_bound_into_the_ciphertext() {
    ...
    let ciphertext = encrypt_secret(
        &manager,
        SecretContext::AccountPortalBindingToken { provider, binding },
        &plaintext,
    )
    .expect("encrypt");
    ...
    for context in [
        SecretContext::AccountPortalOperation { provider, binding },
        SecretContext::AccountPortalRevocation { provider, binding },
        SecretContext::AccountPortalProvider(provider),
    ] {
        assert!(
...
fn old_context_aad_bytes_are_unchanged() {
    ...
    for context in [
        SecretContext::AccountPortalProvider(Uuid::new_v4()),
        SecretContext::AccountPortalBindingToken {
            provider: Uuid::new_v4(),
            binding: Uuid::new_v4(),
        },
    ...
    let old_account_provider = manager
        .encrypt_for(
            SecretContext::AccountProvider(existing),
            "old-settings-token",
        )
        .expect("encrypt");
    assert_eq!(
...
#[tokio::test]
async fn account_portal_key_lives_in_its_own_subdirectory_with_its_own_key() {
    let root = temp_root();
    ...
    let context = SecretContext::AccountPortalProvider(Uuid::new_v4());
    let ciphertext =
