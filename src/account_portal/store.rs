//! Account Provider v1 消费方 SQL 出口。只碰 0058 的四张新表。

use crate::sqlx::{PgPool, PgRow, Row};
use serde_json::Value;
use uuid::Uuid;

use super::types::{BindingRow, OperationRow, OutboxRow, ProviderRow};
...
pub const ISSUER_UNIQUE: &str = "account_portal_providers_issuer_key";
pub const LIVE_PROVIDER_USER: &str = "account_portal_bindings_live_provider_user";
pub const LIVE_PROVIDER_UID: &str = "account_portal_bindings_live_provider_uid";
pub const OPERATION_PKEY: &str = "account_portal_operations_pkey";
pub const OUTBOX_BINDING_UNIQUE: &str = "account_portal_revocation_outbox_binding_key";
...
pub enum StoreError {
    #[error("account portal provider issuer is already registered")]
    IssuerTaken,
    #[error("account portal user already has a live binding on this provider")]
    UserSlotTaken,
    #[error("account portal uid is already bound on this provider")]
    UidTaken,
    #[error("account portal row was changed concurrently")]
    Conflict,
    #[error("account portal database operation failed")]
...
pub async fn has_persisted_ciphertext(pool: &PgPool) -> Result<bool, crate::sqlx::Error> {
    crate::sqlx::query_scalar(
        "SELECT EXISTS (
             SELECT 1 FROM account_portal_providers
             WHERE octet_length(client_secret_ciphertext) > 0
         ) OR EXISTS (
             SELECT 1 FROM account_portal_bindings
