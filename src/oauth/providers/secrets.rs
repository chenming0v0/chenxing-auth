...
pub enum SecretContext {
    Provider(i64),
    Smtp,
    /// 资源服务（Account Provider v1 消费方）客户端配置（`client_secret`）。AAD 只含 provider UUID。
    ResourceServiceProvider(uuid::Uuid),
    /// Account Provider v1 可逆令牌包（access + refresh）。AAD = provider UUID + binding UUID。
    ResourceServiceBindingToken {
        provider: uuid::Uuid,
        binding: uuid::Uuid,
    },
    /// Account Provider v1 操作上下文（在途创建/刷新的持久化操作状态）。
    ResourceServiceOperation {
        provider: uuid::Uuid,
        binding: uuid::Uuid,
    },
    /// Account Provider v1 撤销任务载荷。与操作上下文分成不同 purpose，密文不可互换。
    ResourceServiceRevocation {
        provider: uuid::Uuid,
        binding: uuid::Uuid,
    },
}
...
impl SecretContext {
    fn aad(self) -> Vec<u8> {
        ...
        match self {
            ...
            Self::ResourceServiceProvider(provider) => {
                aad.extend_from_slice(b"resource-service\0provider\0");
                aad.extend_from_slice(provider.as_bytes());
            }
            Self::ResourceServiceBindingToken { provider, binding } => {
                resource_service_pair_aad(&mut aad, b"binding-token\0", provider, binding);
            }
            Self::ResourceServiceOperation { provider, binding } => {
                resource_service_pair_aad(&mut aad, b"operation\0", provider, binding);
            }
            Self::ResourceServiceRevocation { provider, binding } => {
                resource_service_pair_aad(&mut aad, b"revocation\0", provider, binding);
            }
        }
        aad
    }
...
/// 追加 `resource-service` 命名空间下「provider + binding」型 AAD。
///
/// purpose 自带结尾 `\0`，两个 UUID 各为固定 16 字节，因此编码无歧义，不需要
/// 用空 UUID 之类的哨兵值来补位。旧变体的字节序列不受本函数影响。
fn resource_service_pair_aad(
    aad: &mut Vec<u8>,
    purpose: &[u8],
    provider: uuid::Uuid,
    binding: uuid::Uuid,
) {
    aad.extend_from_slice(b"resource-service\0");
