...
impl KeyManager {
    ...
    pub fn load_or_generate_with_retention(
        ...
        cleanup_expired_key_files(&directory, active_id.as_deref(), retention)?;
        ...
        if key_files.is_empty() {
            ...
            let created_at = modified_time(&directory.join(key_file_name(&key_id)))?;
        ...
        prune_materials(Some(&directory), &active_key_id, &mut key_files, retention);
    ...
    pub async fn rotate(&self) -> Result<KeyRotation, KeyManagerError> {
        ...
        tokio::task::spawn_blocking(move || manager.rotate_blocking())
            .await
    ...
    fn rotate_blocking(&self) -> Result<KeyRotation, KeyManagerError> {
        ...
        let (key_id, der) = generate_rsa_key()?;
        prune_materials(directory.as_ref(), &key_id, &mut materials, retention);
        materials.insert(key_id.clone(), key_material(der.clone(), SystemTime::now()));
        ...
        if let Some(directory) = directory.as_ref()
            && let Err(error) = cleanup_expired_key_files(directory, Some(&key_id), retention)
        {
    ...
    fn from_key_material(
        ...
        let materials = materials
            .into_iter()
            .map(|(key_id, der)| (key_id, key_material(der, SystemTime::now())))
            .collect();
        Self::from_materials(
            directory,
            Duration::from_secs(DEFAULT_KEY_RETENTION_SECONDS),
...
fn cleanup_expired_key_files(
    ...
    for entry in fs::read_dir(directory)? {
        ...
        validate_key_id(key_id)?;
        let created_at = modified_time(&path)?;
        if active_key_id != Some(key_id) && !within_retention(created_at, retention) {
            fs::remove_file(path)?;
        }
    }
    Ok(())
...
fn migrate_legacy_key(
    ...
    key_files.insert(key_id, key_material(der, SystemTime::now()));
    Ok(())
}
...
fn prune_materials(
    ...
    materials.retain(|key_id, material| {
        key_id == active_key_id || within_retention(material.created_at, retention)
    });
}

fn within_retention(created_at: SystemTime, retention: Duration) -> bool {
    SystemTime::now()
