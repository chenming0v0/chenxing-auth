...
/// 认证失败计数窗口上限（秒）。滑动窗口 24 小时意味着失败计数要静默整天才清零，
/// 已经是实际可用的最严策略；更长只会长期锁死正常用户，并让计数键长期驻留 Redis。
...
/// `$check` 宏逐字段展开。
///
/// 只有清单是共享的，动作不共享：启动期环境变量越界回退默认值并告警；已持久化
/// 值走统一 decode / 校验（热路径 fail-closed，管理读取返回可修复诊断）；管理
/// API 写入越界直接拒绝并回报字段名。因此 `$check` 自己写 `< 1 || > $max` 的判断
/// 和后续动作，这里只保证「没有任何入口能漏掉字段」。
