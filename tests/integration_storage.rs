...
async fn insert_session_test_user(pool: &chenxing_auth::sqlx::PgPool, label: &str) -> i64 {
    let suffix = Uuid::new_v4().simple().to_string();
...
async fn session_save_persists_one_identity_across_payload_row_and_outbox() {
    let pool = database().await;
    let user_id = insert_session_test_user(&pool, "session-payload-identity").await;
    let key = session_store_key();
    let store = SessionStore::with_metadata_and_key(redis_client(), pool.clone(), key);
    ...
    let payload = decrypt_session_payload_json(&encrypted_payload, key);
    assert_eq!(
...
async fn concurrent_session_saves_keep_unique_ids_matched_to_each_payload() {
    let pool = database().await;
    let user_id = insert_session_test_user(&pool, "session-payload-concurrent").await;
    let key = session_store_key();
    let store = SessionStore::with_metadata_and_key(redis_client(), pool.clone(), key)
...
async fn session_save_failure_rolls_back_row_and_outbox_without_publishing_id() {
    let pool = database().await;
    let user_id = insert_session_test_user(&pool, "session-payload-rollback").await;
