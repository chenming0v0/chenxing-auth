...
use chenxing_auth::{
    api, auth_factors::domain::FactorMethod, clock::SharedClock, sessions::cookies,
...
async fn setup() -> (
    ...
    let (state, database, key_directory, _admin_token, binary_name) =
        harness::HarnessBuilder::new("totp_auth")
...
async fn totp_setup_confirm_issues_session_and_consumes_ticket() {
    ...
    assert_eq!(response.status(), StatusCode::OK);
    assert!(response.headers().get("set-cookie").is_some());
    let session_body = http::json_body(response).await;
    ...
    assert_eq!(http::json_body(response).await["status"], "factor_required");
    ...
    .await;
    let second_cookie = http::set_cookies(&login_response);
    let _pending = http::json_body(login_response).await;
    let response = request_with_cookie(
        &router,
