...
- 默认把完整验证交给 CI：提交前只做必要的轻量检查，不要为了重复 CI 而先在本地运行全量测试。
...
- 后端仅允许静态托管 React 构建产物（`web/dist`）用于单二进制部署，不得输出自定义 HTML 页面或服务端渲染内容
...
- `@chenxing/ui` 已提供的通用 UI 组件（原语/select/drawer/data-table/motion/space/skip-link/toast/topbar/error-boundary/intro-gate/avatar-editor 等）必须直接导入使用，不得在本项目重建同名组件；`web/src/components` 只保留业务壳层（shells），业务专属组件按页面或业务归属放置。
- 如果所需组件在 `@chenxing/ui` 尚未提供，允许先在本项目 `web/src` 实现临时回退 UI，使用项目现有的卡片/面板视觉语言并编写该回退组件所需的应用侧样式。这里没有预设的“回退卡片”组件，也不要求先修改组件库；临时实现只用于承接当前功能，不能伪装成 `@chenxing/ui` 的正式通用组件。
- 采用临时回退时，先检查本仓库和 `chenming0v0/chenxing-ui` 是否已有对应 issue；没有则分别创建 issue，记录缺少的组件、使用场景和正式组件建议，互相链接，并在任务或 PR 说明中记录 issue 编号。正式组件可用后，应移除临时实现并切换到 `@chenxing/ui`。
- 正式卡片、玻璃容器、表单面板和对话框仍应优先通过项目级 `chenxing-hud-panel` skill 指定的公共容器 `.chenxing-hud-panel`，并通过 `@chenxing/ui` 导出的 `HudPanel` 渲染。临时回退可以编写必要的局部卡片/组件样式，但不得复制或改写 `HudPanel` 的公共 CSS 配方，也不得另建一套长期通用的玻璃规范；禁止复用旧的 `chenxing-glass-strong chenxing-hud-frame` 组合。
- 正式面板样式的唯一来源是组件库 `@chenxing/ui` 的 `styles.css`（仓库 https://github.com/chenming0v0/chenxing-ui ，经 `web/src/index.css` 引入）；不要复制到本仓库 CSS 或组件内联样式。临时回退的组件专属样式可以放在应用侧，但应随正式组件落地删除；应用专属外壳样式只放 `web/src/app-shell.css`。需要修改正式通用组件时，去组件库仓库改并更新依赖锁定。

### 变更验收

- 完成任何源代码修改后，无论是否创建 Git commit，在准备报告任务完成前都必须使用项目级 `src-line-limit` skill 检查 `src` 目录：超过 300 行属弱警告，必须在变更说明中记录；超过 500 行属强警告，完成前必须拆分或重构，除非用户明确接受例外，不得声称任务完成。
...
需要确认测试目标能编译时，编排者可按需使用 `cargo check --tests` 或 `cargo check --all-targets`。这两个命令只编译不跑用例，但都会产生 `target/` 文件；任何子代理均不得执行。它们是本地编译反馈，不替代 CI 的完整验证；默认优先 `cargo check`，确有需要再扩大范围，不要无脑同时运行多个检查。
...
`cargo test`、`cargo nextest run`、`cargo llvm-cov` 都执行用例。默认不要绕过 CI 在本地运行全量；只有下文规定的 CI 故障复现例外，才可在获得授权后运行。
...
`--full`、`--gate`、`--coverage` 属于全量，**不因改动风险高就自动在本地运行，也不应在首次推送 CI 前申请授权**。标准流程是完成改动、做必要的轻量检查后提交并推送，让 CI 执行一次完整验证；必要的 `--lib` / `--test NAME` 聚焦测试可以用于本地快速反馈，但不替代 CI。

只有同时满足以下条件时，才向用户请求一次本地全量测试授权：

- CI 已经失败，且仅靠 CI 日志无法定位或修复问题，需要本地复现 `--full`、`--gate` 或 `--coverage` 的行为。
- 本地复现确实有助于验证修复，而不是为了在推送前重复 CI。
- 请求时机是在查看 CI 失败结果并完成可行的远程修复尝试之后。

- 一次授权只对当次这一次运行有效，不延伸到后续任务、子代理或覆盖率命令。
- 全量跑出问题由编排者按日志继续修复，不转交子代理反复跑全量。
- 默认不在本地跑全量时，完整测试、数据库集成测试和高风险流程验证均由 CI 覆盖；这不是验证缺失，而是项目的标准流程。
...
- 合并 dev 到 releases 后看到 CI 和 Build And Publish 都成功，**不等于发布完成**：分支 push 触发的那次运行不含 `Publish release assets` job（该 job 只响应 tag push）。必须确认远端存在 `vX.Y.Z` 标签且 Build And Publish 有 tag 事件的运行后，GitHub Release 才会出现在列表。
- `design` 是设计稿分支，只保存 `design-auth-chengming/` 和设计稿专用 skill；它单向从 `dev` 接收更新，禁止合并回 `dev` 或 `releases`。
- 未明确要求使用子代理工作树或功能分支时，普通任务直接在 `dev` 修改，不主动创建或切换到新分支。只有明确要求时才创建功能分支或工作树，并从指定的目标基线创建；开始工作前必须检查当前分支、工作区状态、远端跟踪关系以及目标分支的祖先关系，确认没有把功能分支误合入 `dev` 或 `releases`。
...
- 只有外部 HTTP 契约发生变化时，才必须调用项目级 `sync-openapi` skill，同步更新 `openapi.yaml`。这里的外部契约包括 HTTP 方法和路径、请求的 Path/Query/Header/Body 字段及校验规则、响应状态码和数据结构、错误码、鉴权要求，以及已纳入接口契约的 OAuth/OIDC 行为。
