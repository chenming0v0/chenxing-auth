...
import { ArrowRight, CheckCircle2, KeyRound, Loader2, LockKeyhole, UserRound } from "lucide-react";
...
export default function Bootstrap({ onComplete }: { onComplete: () => void }) {
  ...
  return <AuthShell title="初始化认证中枢" subtitle="设置第一个管理员账户，开始管理辰星身份服务">
    {success ? <div className="mt-8 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-5 text-center" role="status">
      ...
      <div className="mt-1 text-xs text-emerald-200/70">即将跳转到管理员登录页</div>
    </div> : <form className="mt-6 space-y-4" onSubmit={submit}>
      <FormError value={error} />
      <Field label="管理员用户名" icon={<UserRound size={15} />} type="text" autoComplete="username" required minLength={3} maxLength={64} placeholder="chenxing-admin" value={username} onChange={(event) => setUsername(event.target.value)} />
      <Field label="管理员密码" icon={<LockKeyhole size={15} />} type="password" autoComplete="new-password" required minLength={10} placeholder="至少 10 个字符" value={password} onChange={(event) => setPassword(event.target.value)} />
      <Field label="确认密码" icon={<KeyRound size={15} />} type="password" autoComplete="new-password" required minLength={10} placeholder="再次输入管理员密码" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} />
